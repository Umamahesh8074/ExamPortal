...
//For Searching
  ...
  byExamId(){
    this.http.get<Marks[]>(`http://localhost:9033/api/marks/1`).subscribe((data)=>{
      console.warn(data)
    })
  }
  ...
  chosenYearHandler(normalizedYear: Moment, dp: any) {
    ...
    this.http.get<ScheduleExam[]>(`http://localhost:9033/api/getexamsbyyear/${normalizedYear.year()}`)
  ...
  onExamSelection(){
    ...
    return this.http.get<Marks[]>(`http://localhost:9033/api/marks/${this.selectedexam}`).subscribe((data)=>{this.marks=data
